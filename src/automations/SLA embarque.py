...
def sla_last_mile():
    ...
    def get_occurence_time(dict_list, oc_list):
        ...
        if type(dict_list) == float or len(dict_list) == 0:
            pass
            return None
        else:
            oc_creation_time = []
    ...
    services.drop(services[services['step'] == 'toAllocateValidate'].index, inplace=True)

    embarques = services

    df_dt += dt.timedelta(days=1)  # Correção para poder buscar somente um dia

    user_fb.set('Organizando informações dos embarques...')  # Tratando dados da coleta

    embarques['REGIONAL'] = embarques['serviceIDRequested.budgetIDService.source_hub_id'].map(get_hub_regional)

    embarques['RESPONSÁVEL'] = np.select(
        condlist=[embarques['REGIONAL'] == 1,
                  embarques['REGIONAL'] == 2,
                  embarques['REGIONAL'] == 3,
                  embarques['REGIONAL'] == 4,
                  embarques['REGIONAL'] == 5],
        choicelist=['Ivan/João', 'Rômulo', 'Rennan', 'Benito', 'Pamella'],
        default=None
    )

    embarques['ESTADO'] = services['serviceIDRequested.budgetIDService.source_hub_id'].map(get_hub_state)
    embarques['UF'] = embarques['ESTADO'].map(get_uf)

    embarques['EMBARQUES'] = embarques['serviceIDBoard'].str[0]

    embarques['boardDatetime'] = embarques['EMBARQUES'].map(
        lambda x: x.get('real_board_date', np.nan) if isinstance(x, dict) else np.nan)

    embarques['boardDatetime'] = pd.to_datetime(embarques['boardDatetime']) - dt.timedelta(hours=6)
    embarques['boardDatetime'] = embarques['boardDatetime'].dt.tz_localize(None)

    embarques.drop(embarques[embarques['boardDatetime'] < di_dt].index, inplace=True)
    embarques.drop(embarques[embarques['boardDatetime'] > df_dt].index, inplace=True)

    embarques['TRANSPORTADORA'] = embarques['serviceIDRequested.source_branch_id'].map(get_transp)

    embarques['dataReal'] = embarques['boardDatetime'].map(excel_date)
    embarques['horaReal'] = embarques['boardDatetime'].map(excel_time)

    embarques['dataEmbarque'] = pd.to_datetime(embarques['serviceIDRequested.board_date']) - dt.timedelta(hours=3)
    embarques['dataEmbarque'] = embarques['dataEmbarque'].dt.tz_localize(None)
    embarques['dataEmbarque'] = embarques['dataEmbarque'].map(excel_date)

    embarques['hourEnd'] = pd.to_datetime(embarques['serviceIDRequested.board_hour']) - dt.timedelta(hours=3)
    embarques['hourEnd'] = embarques['hourEnd'].dt.tz_localize(None)
    embarques['hourEnd'] = embarques['hourEnd'].map(excel_time)

    embarques['datahoraLimite'] = embarques['dataEmbarque'] + embarques['hourEnd']

    embarques['datahoraReal'] = embarques['dataReal'] + embarques['horaReal']

    # embarques['datahoraReal'] = np.select(
    #     condlist=[embarques['UF'] == 'AC',
    #               (embarques['UF'] == 'AM') | (embarques['UF'] == 'RR') | (embarques['UF'] == 'RO') | (embarques['UF'] == 'MT')
    #               | (embarques['UF'] == 'MS')],
    #     choicelist=[embarques['datahoraReal'] - excel_time(pd.to_datetime('02:00')),
    #                 embarques['datahoraReal'] - excel_time(pd.to_datetime('01:00'))],
    #     default=embarques['datahoraReal']
    # )

    embarques['ATRASO'] = np.where(
        embarques['datahoraReal'] <= (embarques['dataEmbarque'] + embarques['hourEnd']),
        'NÃO',
        'SIM'
    )

    embarques.sort_values(
        by="boardDatetime", axis=0, ascending=True, inplace=True, kind='quicksort', na_position='last')

    user_fb.set('Atualizando dados...')

    embarques_sla['PROTOCOLO'] = embarques['protocol']
    embarques_sla['CLIENTE'] = embarques['customerIDService.trading_firstname']
    embarques_sla['CIDADE ORIGEM'] = embarques['serviceIDRequested.source_address_id'].map(get_address_city)
    embarques_sla['CIDADE DESTINO'] = embarques['serviceIDRequested.destination_address_id'].map(get_address_city)
    embarques_sla['COLETADOR ORIGEM'] = embarques['serviceIDRequested.source_collector_id'].map(get_collector)
    embarques_sla['TRANSPORTADORA'] = embarques['TRANSPORTADORA']
    embarques_sla['HORA LIMITE DE EMBARQUE'] = embarques['datahoraLimite']
    embarques_sla['HORA COLETADO'] = embarques['datahoraReal']
    embarques_sla['ATRASO'] = embarques['ATRASO']
    embarques_sla['UF'] = embarques['UF']
    embarques_sla['RESPONSÁVEL'] = embarques['RESPONSÁVEL']
    ...
    export_to_excel(embarques_sla, file_name, 'Embarques', 'A1:K20000')
    time.sleep(0.1)
